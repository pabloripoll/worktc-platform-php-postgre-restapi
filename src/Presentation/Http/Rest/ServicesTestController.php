...
class ServicesTestController extends AbstractController
{
    ...
    public function testAll(MailerInterface $mailer): JsonResponse
    {
        ...
        $data = [
            ...
            'status' => [
                'database' => ($postgresData['success'] ?? false) ? true : false,
                'mailer' => ($mailerData['success'] ?? false) ? true : false,
                'broker' => ($brokerData['success'] ?? false) ? true : false,
                'redis' => ($redisData['success'] ?? false) ? true : false,
                'mongodb' => ($mongoData['success'] ?? false) ? true : false,
            ],
            'status_all' => $allSuccess,
            'timestamp' => (new \DateTime())->format(\DateTime::ATOM),
            'results' => $results,
        ];
